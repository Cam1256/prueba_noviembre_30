...
def fidelityplot(
    filepath: str = typer.Option(..., help="Filepath for the input CSV File"),
    delimiter: str = typer.Option(..., help="Delimiter of the classes in the input CSV File"),
    outfilename: str = typer.Option(..., help="Name of the output file without extensions"),
):
    
    with open(filepath) as inf:
        reader = csv.reader(inf.readlines(), delimiter=delimiter)
        max_col=0
        for line in reader:
    ...
    suffix = ".html"
    path = dirname +filename + suffix
    net.show(path)

    
    
if __name__ == "__main__":
    typer.run(fidelityplot)